...
.post(function(req, res){
  ...
  form.parse(req, (err, fields, files) => {
    ...
    const {id,client,discription,videoLink,publish} = fields
    console.log({id,client,discription,videoLink,publish})
    const proposal = new Proposal({id,client,discription,videoLink,publish: publish=='true'?'true':'false'})
    proposal.save()
    .then(()=>{
        res.status(200).json(proposal)
    })
